...
function processRandomResponse(xhr){
	var response = 'error';
	if(xhr.readyState == 4) { // done
		if (xhr.status == 200) {
			// success
			response = xhr.response;
			window.localStorage.setItem('randomOrg_Count', num);
...
// pokemon max = 721
function requestNewRandomSet(number, min, max, callback){
